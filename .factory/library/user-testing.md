...
- `scripts/parity/run_matrix.py` has CLI groups `cli-formatting`, `cli-exit-errors`, and `cli-count-stream`; `cli-count-stream` artifacts cover multi-file, single-file, and stdin/stream scenarios for full `VAL-CLI-001..003` evidence via `--group cli-count-stream`.
